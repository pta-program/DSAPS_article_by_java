...
public class Main {
    static final int ERROR = 100000000;
    static class TwoStack {
        int[] data; int top1, top2;
        TwoStack(int n) { data = new int[n]; top1 = -1; top2 = n; }
    }

    static TwoStack CreateStack(int maxSize) { return new TwoStack(maxSize); }
    ...
    static int Pop(TwoStack s, int tag) {
        if (tag == 1) {
            if (s.top1 < 0) { System.out.print("Stack 1 Empty"); return ERROR; }
            return s.data[s.top1--];
        }
        if (tag == 2) {
            if (s.top2 == s.data.length) { System.out.print("Stack 2 Empty"); return ERROR; }
