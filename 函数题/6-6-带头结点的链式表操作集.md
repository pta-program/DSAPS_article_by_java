...
public class Main {
    ...
    static ListNode MakeEmpty() { return new ListNode(0); }
    ...
    static boolean Insert(ListNode L, int x, ListNode p) {
        ...
        if (prev.next != p) { System.out.println("Wrong Position for Insertion"); return false; }
        ListNode node = new ListNode(x);
        node.next = p; prev.next = node;
        return true;
    }

    static boolean Delete(ListNode L, ListNode p) {
        if (L == null || p == null) { System.out.println("Wrong Position for Deletion"); return false; }
        ListNode prev = L;
        while (prev.next != null && prev.next != p) prev = prev.next;
        if (prev.next != p) { System.out.println("Wrong Position for Deletion"); return false; }
        prev.next = p.next;
        return true;
    }
