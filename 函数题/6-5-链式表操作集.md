...
public class Main {
    ...
    static ListNode Insert(ListNode L, int x, ListNode p) {
        ...
        if (prev == null) { System.out.println("Wrong Position for Insertion"); return null; }
        node.next = p; prev.next = node;
        return L;
    }

    static ListNode Delete(ListNode L, ListNode p) {
        if (L == null || p == null) { System.out.println("Wrong Position for Deletion"); return null; }
        if (L == p) return L.next;
        ListNode prev = L;
        while (prev != null && prev.next != p) prev = prev.next;
        if (prev == null) { System.out.println("Wrong Position for Deletion"); return null; }
        prev.next = p.next;
        return L;
    }
