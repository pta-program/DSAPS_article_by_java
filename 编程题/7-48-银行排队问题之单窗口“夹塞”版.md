...
static class Customer {
    String name; int arrive, process, group;
    Customer(String name, int arrive, int process, int group) {
        this.name = name; this.arrive = arrive; this.process = Math.min(process, 60);
        this.group = group;
    }
}
...
static double averageWait(List<Customer> customers) {
    ...
    while (next < customers.size() || !queue.isEmpty()) {
        ...
        for (int i = index + 1; i < customers.size(); i++) {
            ...
            if (!done[i] && c.group != 0 && friend.group == c.group
                    && friend.arrive <= ownEnd) {
                queue.remove(i);
                inQueue[i] = false;
                friends.add(i);
            }
