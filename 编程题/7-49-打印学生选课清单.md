...
```java
// 实现原理：建立“学生 → 课程列表”的倒排表。逐门课程读取名单，把课程编号加入对应学生的列表；查询时按课程编号升序输出该学生的所有课程。
static Map<String, List<Integer>> coursesByStudent(Map<Integer, List<String>> courseStudents) {
