...
static class Activity {
    ...
    Activity(int from, int to, int time, int order) {
        this.from = from; this.to = to; this.time = time; this.order = order;
    }
}
...
static List<Activity> criticalActivities(int n, List<Activity>[] g) {
    ...
    for (List<Activity> es : g)
        for (Activity e : es)
            if (ve[e.from] == vl[e.to] - e.time) ans.add(e);
    ans.sort((a, b) -> a.from != b.from
        ? Integer.compare(a.from, b.from)
        : Integer.compare(b.order, a.order));
    return ans;
}
```
### 代码流程图
