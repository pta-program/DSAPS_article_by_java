...
```java
// 实现原理：建立“课程 → 学生名单”的倒排表。读取每位学生的课程编号并加入相应列表，最后对每门课的名单按字典序排序后输出。
static Map<Integer, List<String>> studentsByCourse(List<String[]> selections, int courseCount) {
  Map<Integer, List<String>> result = new HashMap<>();
  for (int i = 1; i <= courseCount; i++) result.put(i, new ArrayList<>());
  for (String[] row : selections) {
    String name = row[0];
    for (int i = 1; i < row.length; i++) result.get(Integer.parseInt(row[i])).add(name);
  }
